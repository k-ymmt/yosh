...
fn spawn_yosh_with_ignored(signals: &[i32], script: &str) -> (String, String, i32) {
    ...
    unsafe {
        cmd.pre_exec(move || {
            ...
            for &num in &sigs {
                ...
                sigaction(sig, &sa).map_err(|e| std::io::Error::from_raw_os_error(e as i32))?;
            }
            Ok(())
        });
    }
    ...
    let out = cmd
        .output()
        .expect("yosh binary should be buildable and runnable");
    (
...
fn trap_set_on_ignored_on_entry_sigint_is_silent() {
    ...
    let (stdout, stderr, code) =
        spawn_yosh_with_ignored(&[libc::SIGINT], "trap 'echo caught' INT; echo $?");
    assert_eq!(code, 0, "exit code; stderr={}", stderr);
    assert_eq!(
        stdout.trim(),
        "0",
        "stdout should be just '0'; got {:?}",
        stdout
    );
...
fn trap_reset_on_ignored_on_entry_sigint_is_silent() {
    ...
    let (stdout, stderr, code) = spawn_yosh_with_ignored(&[libc::SIGINT], "trap - INT; echo $?");
    assert_eq!(code, 0, "exit code; stderr={}", stderr);
...
fn trap_display_shows_ignored_on_entry() {
    ...
    let (stdout, _stderr, code) = spawn_yosh_with_ignored(&[libc::SIGTERM], "trap");
    assert_eq!(code, 0);
    assert!(
...
fn external_cmd_inherits_ignored_on_entry() {
    ...
    let (stdout, _stderr, code) =
        spawn_yosh_with_ignored(&[libc::SIGINT], "sh -c 'kill -INT $$; echo still_alive'");
    assert_eq!(
        code, 0,
        "external sh should survive SIGINT (SIG_IGN preserved); stdout={:?}",
        stdout
    );
    assert!(
