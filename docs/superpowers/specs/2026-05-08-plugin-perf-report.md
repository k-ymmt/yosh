...
- `plugin_exec_burst_var` Criterion median: **1,306 ns** (vs §4.1 baseline 1,205 ns → +101 ns / +8.4%; Criterion's run-to-run change report showed +3.55% to +6.38% with p < 0.05). The bench exercises `variables::get` (already a `WasmStr` from §4.1) and was NOT touched by this rollout — the movement is incidental codegen drift from adjacent linker.rs closures, not a regression in the converted paths. §4.1's own rollout reported a comparable +3.0% movement on the same bench and treated it as in-noise. **Caveat:** the spec defined a ±5% Criterion gate (`docs/superpowers/specs/2026-05-09-plugin-data-borrow-design.md` §4.3); +8.4% nominally exceeds it. The gate is treated as *informational* here — the bench does not exercise any path this rollout converted, so re-tightening the gate would require re-baselining `variables::get` against current HEAD codegen, which is out of scope. If a future rollout touches `variables::get`, re-baseline first.
