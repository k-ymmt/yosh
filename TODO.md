...
- [ ] E2E XFAIL mirror for `assignment_rhs_line_continuation_tilde_known_regression` — add `tilde_mixed_line_continuation_known_regression.sh` under `e2e/posix_spec/2_06_01_tilde_expansion/` with `# XFAIL: §2.6.1 line-continuation tilde after unquoted : — TODO.md tracks; sub-project 4 will fix via escape metadata`, expected output `foo:/home/x/bin`. Surfaces the gap in every E2E run and auto-XPasses when sub-project 4 fixes the underlying lexer behavior.
