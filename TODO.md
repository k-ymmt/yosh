...
- [ ] Timeout-kill tests (production `src/plugin/host/commands.rs` and
      mirror `test_host/commands.rs`) still cannot detect a deleted
      `child.wait()` (zombie left behind): the survivor pgrep check in
      `exec_kills_term_ignoring_child_on_timeout` catches a missing
      SIGKILL but macOS pgrep does not match zombies. Asserting "PID
      fully reaped" needs the spawn helpers to expose the child pid.
      Wrap-up review 2026-08-25 round 3 residual
      (`crates/yosh-plugin-manager/src/test_host/commands.rs`).
- [ ] Spec §8.4 "metadata cannot reach host APIs" — covered at the host-internal level via the per-module `*_denied_when_env_null` tests in `src/plugin/host/` (every real host import returns `Err(Denied)` when `HostContext.env` is null). A contrived plugin whose `metadata()` calls `cwd()` would test the same invariant but requires SDK plumbing to override the trait's default `metadata` body, which Task 6 deferred. If the SDK gains an `override_metadata` hook in the future, add the integration-level companion.
...
- [ ] `files:write` host ops (`write-file`/`append-file`/`create-dir`) collapse `io::ErrorKind::NotFound` into `IoFailed` rather than mapping to `ErrorCode::NotFound` like the read ops do. Spec §4 error-mapping table is written as if uniform across all 8 functions; either add a footnote acknowledging the write-side asymmetry or actually map NotFound on the write side (e.g., parent-dir-missing on `write-file`/`create-dir`). Pre-decided as acceptable during implementation but worth revisiting if a plugin author wants the parent-not-found distinction (`src/plugin/host/files.rs`, `docs/superpowers/specs/2026-04-29-plugin-files-rw-capability-design.md` §4). Code-review follow-up from 2026-04-29 plugin files-rw branch.
